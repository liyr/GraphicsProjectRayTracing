...
inline void cross_product(double* v1, double* v2, double* out)
{
...
inline double mix_product(double* v0, double* v1, double* v2)
{
    double out[3];
    out[0] = v1[1] * v2[2] - v1[2] * v2[1];
    out[1] = v1[2] * v2[0] - v1[0] * v2[2];
...
inline void plus(double* v, const double* v1)
{
    v[0] = v[0] + v1[0];
    v[1] = v[1] + v1[1];
    v[2] = v[2] + v1[2];
...
inline void std_coords_to_half_diff_coords(double theta_in, double fi_in, double theta_out, double fi_out,
    double& theta_half, double& fi_half, double& theta_diff, double& fi_diff)//, double* normal, double* bi_normal)
{
...
inline void lookup_brdf_val(double* brdf, double theta_in, double fi_in,
    double theta_out, double fi_out,
    double& red_val, double& green_val, double& blue_val)//, double* normal, double* bi_normal)
{
    // Convert to halfangle / difference angle coordinates
    double theta_half, fi_half, theta_diff, fi_diff;

    std_coords_to_half_diff_coords(theta_in, fi_in, theta_out, fi_out,
        theta_half, fi_half, theta_diff, fi_diff);//, normal, bi_normal);
...
// Read BRDF data
inline bool read_brdf(const char *filename, double* &brdf)
{
    FILE *f;
